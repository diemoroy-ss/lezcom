...
export default function Contacto() {
  ...
  const [mensajeStatus, setMensajeStatus] = useState<{ type: "success" | "error"; text: string } | null>(null);
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (respuesta.ok) {
        setMensajeStatus({
          type: "success",
          text: "¡Gracias por tu solicitud! Nos pondremos en contacto contigo a la brevedad."
        });
            ...
            <form onSubmit={manejarEnvio} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              {mensajeStatus && (
                <div style={{
                  ...
                  fontSize: "0.9rem",
                  fontWeight: "600",
                  backgroundColor: mensajeStatus.type === "success" ? "#dcfce7" : "#fee2e2",
                  color: mensajeStatus.type === "success" ? "#15803d" : "#b91c1c",
                  border: `1px solid ${mensajeStatus.type === "success" ? "#bbf7d0" : "#fecaca"}`,
                  textAlign: "center"
                }}>
                  {mensajeStatus.text}
                </div>
